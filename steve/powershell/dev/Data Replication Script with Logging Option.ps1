...
If ((Test-Path -Path $Folderpath))
{
        ...
        {
             $filesize = Get-ChildItem $FilePath

             if ($filesize.Length -gt 100kb )
             {
                 Remove-Item $filesize.FullName -Force
                 
                 New-Item $logfolder\$Logname -ItemType file -Force
             } 
             else
             {
                # Do nothing
...
function Copy-LatestFile
{
    ...
    if($File1Date -gt $File2Date)
    {
        ...
        {
            Add-Content -Path $FilePath -Value "$time Error $errormap"; $errorcheck = "1"
        }
    }
...
if ($errorcheck -ne '1')
{
    Add-Content -Path $FilePath -Value "$time Data Replication completed successfully`n"
}
else
{
    Add-Content -Path $FilePath -Value "$time Data Replication completed with some error - Please check the log for details`n"
}
