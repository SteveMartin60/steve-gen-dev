...
#FunctionConfig
$FunctionConfig=@{OsVersion=          '-Class Win32_OperatingSystem -Property Version'
...
$ManualNamespace=@{wmiMonitorID                         ='-Namespace Root\wmi'
...
#################################################################################################################################
#Config Switch Param
$SwitchConfig=@{OSInfo         ="OsVersion","OSArchitecture","OsCaption","OsInstallDate","OsUpTime","OsProductKey","OsLoggedInUser","OsActivationStatus",'OsAdministrators'
...
[scriptblock]$SbOsAdministrators= `
{
    try
    {
        $LangAdminGroups=@{#Key https://msdn.microsoft.com/ru-ru/library/ms912047(v=winembedded.10).aspx Value Administrators Group
                           "1049"="Администраторы"
                           "1033"="Administrators"
                          }
...
{
    $ManufacturerHashTable = @{"AAC" =	"AcerView";
                               "ACR" = "Acer";
                               "AOC" = "AOC";
                               "AIC" = "AG Neovo";
                               "APP" = "Apple Computer";
...
$ResultProp=@{"GetStringValue"="Svalue"
              "GetBinaryValue"="Uvalue"
              "GetDWORDValue"="UValue"
             }

$ErrorCode=@{"1"="Value doesn't exist"
             "2"="Key doesn't exist"
...
$hk=@{"HKEY_CLASSES_ROOT"=2147483648
