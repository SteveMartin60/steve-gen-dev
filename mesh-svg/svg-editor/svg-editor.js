...
var idStringBB            = 'svg-bb';
var idStringComponent     = 'svg-component';
var idStringPinDiagram    = 'svg-pin-diagram';
var idStringBlockDiagram  = 'block-diagram';
var idStringEditorTools   = 'editor-tools';
var idStringMX            = 'mxx';
var idStringTooltip       = 'tooltip';
var coordinates;
var elementX         ;
var elementY         ;
var infoBox          ;
...
//..............................................................................
function findContentElementsSVG()
{
    ...
    for (var i = 0; i < svgObjects.length; i++)
    {
        ...
        if (subDoc)
        {
            ...
            for (var x = 0; x < elementsAll.length; x++)
            {
                var element = elementsAll[x];

                var eString = getElementString(element);

                if (element.getAttribute('content_type') === idStringComponent || element.getAttribute('content_type') === idStringBlockDiagram)
                {
                    captionContentString = element.getAttribute('caption_en');

                    contentCaption.firstChild.data   = captionContentString;
                }
            }

            addEventListeners();

            if (root.getAttribute('id') === idStringBB)
            {
                ...
                contentType = contentSettings.type.component;

                diagram.setAttribute('viewBox', '-241 -171 482 342');

                findBounds();
            }

            if (root.getAttribute('content_type') === idStringComponent || root.getAttribute('diagramType') === idStringPinDiagram)
            {
                console.log('Found ID:', root.getAttribute('id'));
                diagram = root;
                ...
                diagram.setAttribute('viewBox', '-24.1 -17.1 48.2 34.2');

                findBounds();
            }

            if (root.getAttribute('content_type') === idStringBlockDiagram)
            {
                ...
                contentType = contentSettings.type.blockDiagram;
            ...
            if (root.getAttribute('id') === idStringMX)
            {
                ...
                contentType = contentSettings.type.mx;
            ...
            if (root.getAttribute('id') === idStringEditorTools)
            {
                console.log('Found Editor');

                editor = root;

                contentCaption = editor.getElementById('contentCaption');

                contentName = editor.getElementById('caption');
            ...
            if (root.getAttribute('id') === idStringTooltip)
            {
    ...
    var settings = contentType;

    tooltipTextX         .setAttribute('font-size'   , settings.fontSize   );
    tooltipTextY         .setAttribute('font-size'   , settings.fontSize   );
    tooltipLabelX        .setAttribute('font-size'   , settings.fontSize   );
    tooltipLabelY        .setAttribute('font-size'   , settings.fontSize   );
    elementsGridTarget[0].setAttribute('x1'          , settings.length * -1);
    elementsGridTarget[0].setAttribute('x2'          , settings.length     );
    elementsGridTarget[0].setAttribute('stroke-width', settings.strokeWidth);
    elementsGridTarget[1].setAttribute('y1'          , settings.length * -1);
    elementsGridTarget[1].setAttribute('y2'          , settings.length     );
    elementsGridTarget[1].setAttribute('stroke-width', settings.strokeWidth);
    elementsGridTarget[2].setAttribute('r'           , settings.radius     );
    elementsGridTarget[2].setAttribute('stroke-width', settings.strokeWidth);

    diagram.appendChild(gridTarget);
    diagram.appendChild(tooltipTextX);
    diagram.appendChild(tooltipTextY);
    diagram.appendChild(tooltipLabelX);
    diagram.appendChild(tooltipLabelY);
...
//..............................................................................
function isOnMajorGrid(location)
{
    var majorGridSize = 10;
...
function mouseMove(evt)
{
    ...
    var targetCoords  = diagram.createSVGPoint();
    ...
    targetCoords .x = element.getAttribute('x') || element.getAttribute('cx');
    targetCoords .y = element.getAttribute('y') || element.getAttribute('cy');

    mouseCoords = mouseCoords.matrixTransform(diagram.getScreenCTM().inverse());

    var gridTargetCoords = mouseCoords;

    elementX.firstChild.data  = mouseCoords.x.toFixed(2);
    elementY.firstChild.data  = mouseCoords.y.toFixed(2);

    var settings = contentType;

    if (mouseCoords.x < -2200)
    {
        tooltipTextX .setAttribute('x', (mouseCoords    .x) + (settings.offsetCoordH) + 200);
        tooltipTextY .setAttribute('x', (mouseCoords    .x) + (settings.offsetCoordH) + 200);
        tooltipLabelX.setAttribute('x', (mouseCoords    .x) + (settings.offsetTipLabelH) + 200);
        tooltipLabelY.setAttribute('x', (mouseCoords    .x) + (settings.offsetTipLabelH) + 200);
    }
    else
    {
        tooltipTextX .setAttribute('x', (mouseCoords    .x) + (settings.offsetCoordH));
        tooltipTextY .setAttribute('x', (mouseCoords    .x) + (settings.offsetCoordH));
        tooltipLabelX.setAttribute('x', (mouseCoords    .x) + (settings.offsetTipLabelH));
        tooltipLabelY.setAttribute('x', (mouseCoords    .x) + (settings.offsetTipLabelH));
    }

    if (mouseCoords.y < -1670)
    {
        tooltipTextX .setAttribute('y', (mouseCoords    .y) + (settings.offsetXV) + 35);
        tooltipTextY .setAttribute('y', (mouseCoords    .y) + (settings.offsetYV) + 35);
        tooltipLabelX.setAttribute('y', (mouseCoords    .y) + (settings.offsetXV) + 35);
        tooltipLabelY.setAttribute('y', (mouseCoords    .y) + (settings.offsetYV) + 35);
    }
    else if (mouseCoords.y > 1670)
    {
        tooltipTextX .setAttribute('y', (mouseCoords    .y) + (settings.offsetXV) - 35);
        tooltipTextY .setAttribute('y', (mouseCoords    .y) + (settings.offsetYV) - 35);
        tooltipLabelX.setAttribute('y', (mouseCoords    .y) + (settings.offsetXV) - 35);
    ...
    {
        tooltipTextX .setAttribute('y', (mouseCoords    .y) + (settings.offsetXV));
        tooltipTextY .setAttribute('y', (mouseCoords    .y) + (settings.offsetYV));
        tooltipLabelX.setAttribute('y', (mouseCoords    .y) + (settings.offsetXV));
        tooltipLabelY.setAttribute('y', (mouseCoords    .y) + (settings.offsetYV));
    }

    gridTarget   .setAttribute("x", gridTargetCoords.x);
    gridTarget   .setAttribute("y", gridTargetCoords.y);

    if (isOnMajorGrid(gridTargetCoords))
    {
        gridTarget.setAttribute('stroke-width', '0.15');
        gridTarget.setAttribute('stroke', 'purple');
    }
    else
    {
        gridTarget.setAttribute('stroke-width', '0.075');
        gridTarget.setAttribute('stroke', 'red');
    }

    tooltipTextX.firstChild.data   = mouseCoords.x.toFixed(2);
    tooltipTextY.firstChild.data   = mouseCoords.y.toFixed(2);
